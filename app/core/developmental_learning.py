...
class SafePracticeSandbox:
    ...
    applies time/output limits and optional POSIX resource caps.

    Keep DEVELOPMENTAL_SANDBOX_EXECUTION_ENABLED=false when stronger isolation
    (container/VM) is required.
    """
...
class DevelopmentalLearningSystem:
    ...
    Adds:
    - learning stages derived from durable progress;
    - curiosity from novelty + low confidence;
    - persistent self-generated learning goals;
    - analogy storage;
    - deliberate micro-practice with a restricted sandbox;
    - progress/XP that changes *how* the agent learns, not its safety rules.
    """
        ...
        try:
            conn.executescript(
                ...
                CREATE TABLE IF NOT EXISTS developmental_profiles (
                    ...
                    analogies INTEGER NOT NULL DEFAULT 0,
                ...
                CREATE INDEX IF NOT EXISTS idx_learning_goals_user_status
                    ON learning_goals(user_id, status, priority, updated_at);
                ...
                CREATE INDEX IF NOT EXISTS idx_dev_practice_goal
                    ON developmental_practice(user_id, goal_id, created_at);
                ...
                CREATE INDEX IF NOT EXISTS idx_dev_analogies_user
                    ON developmental_analogies(user_id, created_at);
                """
            )
            conn.commit()
        finally:
    ...
    def _ensure_profile(self, user_id: str) -> None:
        ...
        self._conn().execute(
            """
            INSERT INTO developmental_profiles(user_id,xp,stage,updated_at)
            VALUES(?,0.0,'seed',?)
    ...
    def _add_xp(self, user_id: str, amount: float) -> dict[str, Any]:
        ...
        row = self._conn().execute(
            "SELECT * FROM developmental_profiles WHERE user_id=?", (user_id,)
        ).fetchone()
        old_stage = str(row["stage"])
        xp = max(0.0, float(row["xp"]) + float(amount))
        stage = self.stage_for_xp(xp)
        self._conn().execute(
            "UPDATE developmental_profiles SET xp=?,stage=?,updated_at=? WHERE user_id=?",
            (xp, stage, time.time(), user_id),
        )
        self._conn().commit()
    ...
    def profile(self, user_id: str) -> dict[str, Any]:
        ...
        open_goals = self._conn().execute(
            "SELECT COUNT(*) c FROM learning_goals WHERE user_id=? AND status='open'", (user_id,)
        ).fetchone()["c"]
        return {
            "enabled": bool(settings.developmental_learning_enabled),
            "user_id": user_id,
            "xp": float(row["xp"]),
            "stage": str(row["stage"]),
            "observations": int(row["observations"]),
            "practice_attempts": int(row["practice_attempts"]),
            "practice_successes": int(row["practice_successes"]),
            "analogies": int(row["analogies"]),
    ...
    def goals(self, user_id: str, status: str = "open", limit: int = 20) -> list[LearningGoal]:
        ...
        if status == "all":
            rows = self._conn().execute(
                "SELECT * FROM learning_goals WHERE user_id=? ORDER BY priority DESC,updated_at DESC LIMIT ?",
                (user_id, limit),
            ).fetchall()
        else:
            rows = self._conn().execute(
                "SELECT * FROM learning_goals WHERE user_id=? AND status=? ORDER BY priority DESC,updated_at DESC LIMIT ?",
    ...
    def create_goal(
        ...
        if not question or _SENSITIVE.search(question):
            return None
        ...
        for row in self._conn().execute(
            ...
            if union and len(terms & existing_terms) / len(union) >= 0.55:
                self._conn().execute(
                    ...
                    (
                        max(float(row["curiosity"]), self._clip(curiosity)),
                        max(float(row["confidence"]), self._clip(confidence)),
                        max(float(row["priority"]), self._clip(0.55 * curiosity + 0.45 * (1.0 - confidence))),
                        time.time(),
                        int(row["id"]),
                    ),
                )
        ...
        cur = self._conn().execute(
            """
            INSERT INTO learning_goals(user_id,title,question,status,priority,curiosity,confidence,created_at,updated_at)
            VALUES(?,?,?,'open',?,?,?,?,?)
            """,
            (user_id, title[:160], question, priority, self._clip(curiosity), self._clip(confidence), now, now),
        )
        self._conn().commit()
    ...
    def observe(self, user_id: str, query: str, cognition: dict[str, Any]) -> dict[str, Any]:
        ...
        curiosity = self._clip(
            float(settings.developmental_curiosity_novelty_weight) * novelty
        ...
        now = time.time()
        self._conn().execute(
            "UPDATE developmental_profiles SET observations=observations+1,updated_at=? WHERE user_id=?",
            (now, user_id),
        )
        self._conn().commit()
        ...
        goal_id = None
        if (
            settings.developmental_auto_goals
            and curiosity >= float(settings.developmental_goal_curiosity_threshold)
            and len(self.goals(user_id, "open", 100)) < int(settings.developmental_max_open_goals)
        ):
            goal_id = self.create_goal(user_id, query, curiosity, confidence)

        stage = self.profile(user_id)["stage"]
        if stage in {"seed", "explorer"}:
            learning_mode = "observe_and_compare"
        elif stage == "apprentice":
            learning_mode = "practice_and_analogy"
        elif stage == "practitioner":
            learning_mode = "generalize_and_test"
        else:
            learning_mode = "teach_back_and_refine"
        ...
        return {
            ...
            "active_goals": [asdict(g) for g in self.goals(user_id, "open", 5)],
        }
    ...
    def record_analogy(
        ...
        if _SENSITIVE.search(str(target_query)) or _SENSITIVE.search(str(mapping)):
            return 0
        now = time.time()
        with self._write_lock:
            cur = self._conn().execute(
                """
                INSERT INTO developmental_analogies(user_id,source_title,target_query,mapping,limitation,usefulness,created_at)
                VALUES(?,?,?,?,?,?,?)
                """,
                (
                    user_id,
                    str(source_title)[:200],
                    str(target_query)[:2000],
                    str(mapping)[:5000],
                    str(limitation)[:2500],
                    self._clip(usefulness),
                    now,
                ),
            )
            self._conn().execute(
                "UPDATE developmental_profiles SET analogies=analogies+1,updated_at=? WHERE user_id=?",
                (now, user_id),
            )
            self._conn().commit()
        return int(cur.lastrowid)
    ...
    def update_from_feedback(self, user_id: str, query: str, reward: float) -> dict[str, Any]:
        ...
        rows = self._conn().execute(
            "SELECT * FROM learning_goals WHERE user_id=? AND status='open' ORDER BY priority DESC,updated_at DESC LIMIT 100",
            (user_id,),
        ).fetchall()
        best = None
        best_score = 0.0
        for row in rows:
            gt = self._terms(str(row["question"]))
            union = terms | gt
        ...
        if best is not None and best_score >= 0.20:
            ...
            self._conn().execute(
                """
                UPDATE learning_goals SET attempts=?,successes=?,confidence=?,status=?,updated_at=?,last_attempt=?
    ...
    def _practice_row(self, row: sqlite3.Row) -> PracticeAttempt:
    ...
    async def practice_goal(self, user_id: str, goal_id: int, provider: Any) -> dict[str, Any]:
        row = self._conn().execute(
            "SELECT * FROM learning_goals WHERE id=? AND user_id=?", (int(goal_id), user_id)
        ).fetchone()
        if row is None:
        ...
        prompt = [
            {
                "role": "system",
                "content": (
                    "Create one small deliberate-practice exercise for an AI learning goal. "
                    "Return ONLY JSON with keys: mode ('concept' or 'python'), exercise, answer, code, expected. "
                    "Use python only for deterministic algorithmic practice that needs no files, network, subprocesses or external packages. "
                    "For concept mode leave code and expected empty. Keep it compact and safe."
                ),
            },
            {
                "role": "user",
                "content": f"Learning goal: {goal.title}\nQuestion/gap: {goal.question}\nCurrent confidence: {goal.confidence:.2f}",
            },
        ]
        ...
        mode = str(item.get("mode") or "concept").strip().lower()
        exercise = str(item.get("exercise") or goal.question).strip()[:5000]
        answer = str(item.get("answer") or "").strip()[:5000]
        code = str(item.get("code") or "").strip()[: int(settings.developmental_sandbox_max_code_chars)]
        ...
        if mode == "python" and code:
            ...
            if sandbox_result.get("executed"):
                ...
                score = 1.0 if sandbox_result.get("ok") and (not expected or expected in stdout) else 0.0
                feedback = (
                    "Sandbox execution matched the expected result."
                    if score >= 1.0
                    else f"Sandbox result did not meet expectation. reason={sandbox_result.get('reason')}"
                )
            else:
                score = 0.5 if sandbox_result.get("ok") else 0.0
                feedback = (
                    "Code passed the restricted sandbox validator; execution is disabled."
                    if sandbox_result.get("ok")
                    else f"Code rejected by sandbox: {sandbox_result.get('reason')}"
                )
        else:
            judge_prompt = [
                {
                    "role": "system",
                    "content": (
                        "Judge a self-practice answer against its exercise. Return ONLY JSON: "
                        '{"score":0.0,"feedback":"..."} where score is 0..1. '
                        "Be strict, concise and focus on correctness."
                    ),
                },
                {"role": "user", "content": f"Exercise: {exercise}\nCandidate answer: {answer}"},
            ]
            judged = await provider.chat(judge_prompt, temperature=0.0, tools=None, tool_choice=None)
            judge_raw = str(judged.get("content") or "") if isinstance(judged, dict) else ""
            js, je = judge_raw.find("{"), judge_raw.rfind("}")
            score, feedback = 0.0, "Could not grade practice."
            if js >= 0 and je >= js:
                try:
                    parsed = json.loads(judge_raw[js : je + 1])
                    score = self._clip(float(parsed.get("score", 0.0)))
                    feedback = str(parsed.get("feedback") or feedback)[:3000]
                except Exception:
                    pass

        success = score >= float(settings.developmental_practice_success_score)
        now = time.time()
        with self._write_lock:
            cur = self._conn().execute(
                """
                INSERT INTO developmental_practice(goal_id,user_id,mode,prompt,artifact,score,success,feedback,created_at)
                VALUES(?,?,?,?,?,?,?,?,?)
                """,
                (goal.id, user_id, mode, exercise, artifact, score, int(success), feedback, now),
            )
            attempts = goal.attempts + 1
            successes = goal.successes + int(success)
            confidence = self._clip(goal.confidence * 0.75 + score * 0.25)
            status = (
                "mastered"
                if successes >= int(settings.developmental_goal_mastery_successes) and confidence >= 0.65
                else "open"
            )
            self._conn().execute(
                """
            ...
            self._conn().execute(
                """
                UPDATE developmental_profiles
                SET practice_attempts=practice_attempts+1,
                    practice_successes=practice_successes+?,
                    updated_at=?
                WHERE user_id=?
                """,
                (int(success), now, user_id),
            )
            self._conn().commit()

        xp_gain = float(settings.developmental_practice_xp) * (0.25 + 0.75 * score)
        progress = self._add_xp(user_id, xp_gain)
        return {
            "ok": True,
    ...
    def practice_history(self, user_id: str, goal_id: int | None = None, limit: int = 50) -> list[PracticeAttempt]:
        ...
        if goal_id is None:
            rows = self._conn().execute(
                "SELECT * FROM developmental_practice WHERE user_id=? ORDER BY id DESC LIMIT ?",
                (user_id, limit),
            ).fetchall()
        else:
            rows = self._conn().execute(
                "SELECT * FROM developmental_practice WHERE user_id=? AND goal_id=? ORDER BY id DESC LIMIT ?",
                (user_id, int(goal_id), limit),
            ).fetchall()
        return [self._practice_row(row) for row in rows]

    def context_text(self, state: dict[str, Any]) -> str:
        goals = state.get("active_goals") or []
        goal_text = "; ".join(
            f"{g.get('title','goal')} (priority={float(g.get('priority',0.0)):.2f}, confidence={float(g.get('confidence',0.0)):.2f})"
            for g in goals[:5]
            if isinstance(g, dict)
        ) or "none"
        return (
            f"Development stage={state.get('stage','seed')}; learning mode={state.get('learning_mode','observe_and_compare')}; "
            f"curiosity={float(state.get('curiosity',0.0)):.3f}; novelty={float(state.get('novelty',1.0)):.3f}; "
            f"confidence={float(state.get('confidence',0.0)):.3f}. Active learning goals: {goal_text}. "
            "Curiosity is a prioritization signal, not an instruction to take unsafe or unauthorized actions. "
